...
const HeroActivityCarousel = forwardRef(function HeroActivityCarousel(
  ...
  return (
    ...
    >
      <div
        ref={chromeRef}
        className="relative rounded-2xl border border-white/18 bg-zinc-950/80 p-4 shadow-2xl ring-1 ring-black/40 sm:p-5 will-change-transform"
      ...
      >
        <div className="mb-4 flex items-end justify-between gap-3">
          <div>
            ...
            <h2 className="font-yeseva-one text-xl text-white sm:text-2xl">
              {title}
            </h2>
            <p className="mt-1 max-w-[14rem] text-sm leading-relaxed text-zinc-300">
              {subtitle}
            </p>
          </div>
          <div className="flex items-center gap-2 text-zinc-300">
            <button
              type="button"
              onClick={goPrev}
              className="flex h-9 w-9 items-center justify-center rounded-lg border border-white/20 bg-white/10 text-white transition hover:border-white/35 hover:bg-white/15 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"
        ...
        <div
          ...
          className="min-h-[380px] overflow-hidden rounded-xl sm:min-h-[400px]"
        >
          <div
          ...
          >
            {slides.map((item, i) => {
              ...
              return (
                ...
                >
                  <article className="rounded-xl border border-zinc-200/90 bg-zinc-50 shadow-[0_1px_0_0_rgba(255,255,255,0.6)_inset,0_12px_32px_-8px_rgba(0,0,0,0.35)] sm:px-6 sm:py-6">
                    <div className="border-b border-zinc-200/90 bg-zinc-100 px-5 py-3 sm:px-6">
                      <div className="flex min-h-[2.75rem] flex-wrap items-start justify-between gap-2">
                        <span className="inline-flex items-center rounded-md bg-primary/12 px-2 py-0.5 text-[11px] font-bold uppercase tracking-wider text-primary">
                          {item.category}
                        </span>
                        <span className="inline-flex max-w-[min(100%,16rem)] items-start gap-1 text-right text-xs font-semibold leading-snug text-zinc-700 sm:max-w-[55%]">
                          <Tag className="mt-0.5 h-3.5 w-3.5 shrink-0 text-zinc-500" aria-hidden />
                    ...
                    <div className="px-5 pb-5 pt-4 sm:px-6 sm:pb-6 sm:pt-5">
                      <div className="flex gap-4">
                        ...
                        <div className="min-w-0 flex-1">
                          <h3 className="line-clamp-2 min-h-[3rem] text-lg font-bold leading-snug text-zinc-900 sm:min-h-[3.25rem] sm:text-xl">
                            {item.headline}
                          </h3>
                          <p className="mt-2 line-clamp-5 min-h-[7rem] text-[15px] leading-relaxed text-zinc-700 sm:min-h-[7.25rem]">
                            {item.body}
                          </p>
                        </div>
                      </div>

                      <div className="mt-4 grid grid-cols-3 divide-x divide-zinc-200 rounded-lg border border-zinc-200 bg-white">
                        {item.metrics.map((m) => (
                      ...
                      <div className="mt-4 flex min-h-[4.25rem] flex-col gap-2 border-t border-zinc-200/90 pt-4 text-sm text-zinc-700 sm:flex-row sm:items-start sm:justify-between">
                        <span className="inline-flex min-w-0 max-w-full items-start gap-1.5 font-medium text-zinc-800 sm:max-w-[48%]">
                        ...
                        <span className=" min-h-[2.75rem] rounded-md bg-zinc-100 px-2 py-1 font-mono text-[11px] leading-snug text-zinc-600 sm:max-w-[48%] sm:text-xs">
                          {item.channel}
                        </span>
                      </div>

                      <p className="mt-3 flex min-h-[2.5rem] items-start gap-1.5 text-xs leading-snug text-zinc-700">
            ...
            {slides.map((_, i) => (
              <button
                ...
                className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                  i === activeIndex ? 'bg-primary' : 'bg-white/30 hover:bg-white/45'
                }`}
              />
            ))}
          </div>
          <span className="shrink-0 tabular-nums text-sm font-semibold text-zinc-300">
